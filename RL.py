...
class RewardModel(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.block3 = nn.Sequential(
            nn.Linear(200, 8),
            nn.ReLU(True),
            nn.Linear(8, self.numbins),
        )

    def forward(self, s, a):
        s = self.block1(s)
        a = self.block2(a)
        return self.block3(torch.cat([s, a]).reshape(-1, 200))
...
# RM = RewardModel(n_observations, 1).to(device)
RM = torch.load(r'pre-trained-reward.ckpt').cuda()
